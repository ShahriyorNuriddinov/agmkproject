...
import { LayoutDashboard, Briefcase, Bell, User, LogOut, Plus } from 'lucide-react';
...
const menuItems = [
  ...
  { icon: Briefcase, label: 'Vakansiyalarim', path: '/hr/vacancies' },
  { icon: Plus, label: 'Yangi vakansiya', path: '/hr/vacancies/create' },
  { icon: Bell, label: 'Bildirishnomalar', path: '/hr/notifications' },
  { icon: User, label: 'Profil', path: '/hr/profile' },
];
      ...
      <SidebarHeader className="border-b border-sidebar-border">
        <div className="flex items-center gap-2 px-2 py-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-sidebar-foreground/60">HR Panel</span>
          </div>
        </div>
      </SidebarHeader>
                ...
                <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton asChild isActive={pathname === item.path} tooltip={item.label}>
                    <Link href={item.path} className="flex items-center gap-2 w-full">
                      ...
                      {item.path === '/hr/notifications' && unreadCount > 0 && (
